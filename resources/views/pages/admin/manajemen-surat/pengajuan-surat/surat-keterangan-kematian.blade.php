        ...
        <div class="flex mx-auto w-[70%] grid grid-cols-1 lg:grid-cols-2 gap-2 lg:gap-16">
            <div class="spaye-y-4">
                ...
                <div class="pb-2">
                    ...
                    <input type="text" id="nama" name="nama" value="{{ $datas->nama }}"
                ...
                <div class="pb-2">
                    <label for="tempat_lahir" class="block text-sm font-normal text-gray-700">Tempat Lahir
                        <input type="text" id="tempat_lahir" name="" value="{{ $datas->tempat_lahir }}"
                            class="bg-white mt-1 block w-full px-3 py-1 border border-gray-600 rounded shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                            placeholder="">
                </div>
                <div class="pb-2">
                    <label for="tanggal_lahir" class="block text-sm font-normal text-gray-700">Tanggal Lahir</label>
                    <input type="date" id="tanggal_lahir" name="" value="{{ $datas->tanggal_lahir }}"
                        class="bg-white mt-1 block w-full px-3 py-1 border border-gray-600 rounded shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                </div>
                <div class="pb-2">
                ...
                <div class="pb-2">
                    ...
                    <input type="text" id="nama" name="pekerjaan" value="{{ $datas->pekerjaan }}"
                        class="bg-white mt-1 block w-full px-3 py-1 border border-gray-600 rounded shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                        placeholder="">
                </div>
                <div class="pb-2">
                    <label for="alamat" class="block text-sm font-normal text-gray-700">Alamat</label>
                    <input type="text" id="alamat" name="alamat" value="{{ $datas->alamat }}"
                        class="bg-white mt-1 block w-full px-3 py-1 border border-gray-600 rounded shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                        placeholder="">
                </div>
                ...
                <div class="pb-2">
                    <label for="nama" class="block text-sm font-normal text-gray-700">Nama Lengkap</label>
                    <input type="text" id="nama" name="" value="{{ $datas->nama }}"
                        class="bg-white mt-1 block w-full px-3 py-1 border border-gray-600 rounded shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                        placeholder="">
                </div>
                <div class="pb-2">
            ...
            <div class="">
                ...
                @if ($datas->status != 'Ditolak')
                    <div class="flex justify-between mt-6 mx-auto w-[100%] pb-4">
                        <x-pop-up-tolak />
                        <form action="{{ route('verif.skk', $datas->id) }}" method="POST">
                            @csrf
                            <button type="submit"
...
<script>
    document.getElementById('submit-reject').addEventListener('click', function() {
        ...
        form.method = 'POST';
        form.action =
        '{{ route('reject.skk', $datas->id) }}'; // Pastikan route ini sudah ada di routes/web.php
