...
<body>
    ...
    <div class="pb-20 pl-4 pr-4 sm:ml-64">
        ...
        <h2 class="text-normal font-light pb-4">Surat Keterangan Penghasilan</h2>
        <div class="flex mx-auto w-[70%] grid grid-cols-1 lg:grid-cols-2 gap-2 lg:gap-16">
            <div class="spaye-y-4">
                {{-- Data 1 --}}
                <h1 class="text-xl font-bold pb-4">Data Orang Tua/Wali</h1>
                <div class="pb-2">
                    <label for="text" class="block text-sm font-normal text-gray-700">Nama Lengkap</label>
                    <input type="text" id="text" name="" value="{{ $datas->nama_lengkap_orang_tua }}"
                ...
                <div class="pb-2">
                    ...
                    <select id="countries"
                        class="bg-white mt-1 block w-full px-3 py-1 border border-gray-600 rounded shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        <option {{ $datas->jenis_kelamin_orang_tua == 'Wanita' ? 'selected' : '' }}>Perempuan</option>
                        <option {{ $datas->jenis_kelamin_orang_tua == 'Pria' ? 'selected' : '' }}>Laki-Laki </option>
                    </select>
                </div>
                <div class="pb-2">
                    <label for="text" class="block text-sm font-normal text-gray-700">Tempat Lahir
                        <input type="text" id="text" name="" value="{{ $datas->tempat_lahir_orang_tua }}"
                            class="bg-white mt-1 block w-full px-3 py-1 border border-gray-600 rounded shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                            placeholder="">
                </div>
                <div class="pb-2">
                    <label for="nama" class="block text-sm font-normal text-gray-700">Tanggal Lahir</label>
                    <input type="date" id="tanggal" name="" value="{{ $datas->tanggal_lahir_orang_tua }}"
                        class="bg-white mt-1 block w-full px-3 py-1 border border-gray-600 rounded shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                </div>
                ...
                <div class="pb-2">
                    ...
                    <select id="countries"
                        class="bg-white mt-1 block w-full px-3 py-1 border border-gray-600 rounded shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        <option {{ $datas->agama_orang_tua == 'Islam' ? 'selected' : '' }}>Islam</option>
                        <option {{ $datas->agama_orang_tua == 'Kristen Protestan' ? 'selected' : '' }}>Kristen </option>
                        <option {{ $datas->agama_orang_tua == 'Hindu' ? 'selected' : '' }}>Hindu</option>
                        <option {{ $datas->agama_orang_tua == 'Buddha' ? 'selected' : '' }}>Budhha</option>
                        <option {{ $datas->agama_orang_tua == 'Konghucu' ? 'selected' : '' }}>Konghucu</option>
                    </select>
                </div>
                <div class="pb-2">
                    <label for="nama" class="block text-sm font-normal text-gray-700">Pekerjaan</label>
                    <input type="text" id="nama" name="" value="{{ $datas->pekerjaan_orang_tua }}"
                        class="bg-white mt-1 block w-full px-3 py-1 border border-gray-600 rounded shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                        placeholder="">
                </div>
                <div class="pb-2">
                    <label for="alamat" class="block text-sm font-normal text-gray-700">Alamat</label>
                    <input type="text" id="alamat" name="" value="{{ $datas->alamat_orang_tua }}"
                        class="bg-white mt-1 block w-full px-3 py-1 border border-gray-600 rounded shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                        placeholder="">
                </div>
                {{-- Data 2 --}}
                ...
                <div class="pb-2">
                    <label for="nama" class="block text-sm font-normal text-gray-700">Penghasilan Orang
                        Tua</label>
                    <input type="text" id="nama" name="" value="{{ $datas->penghasilan_orang_tua }}"
                        class="bg-white mt-1 block w-full px-3 py-1 border border-gray-600 rounded shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                        placeholder="">
                </div>
            </div>
            ...
            <div class="">
                ...
                @if ($datas->status != 'Ditolak')
                    <div class="flex justify-between mt-6 mx-auto w-[100%] pb-4">
                        <x-pop-up-tolak />
                        <form action="{{ route('verif.skpot', $datas->id) }}" method="POST">
                            @csrf
                            <button type="submit"
...
<script>
    document.getElementById('submit-reject').addEventListener('click', function() {
        ...
        form.method = 'POST';
        form.action ='{{ route('reject.skpot', $datas->id) }}'; // Pastikan route ini sudah ada di routes/web.php
