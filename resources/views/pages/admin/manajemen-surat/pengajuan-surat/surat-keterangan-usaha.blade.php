            ...
            <div class="flex mx-auto w-[70%] grid grid-cols-1 lg:grid-cols-2 gap-2 lg:gap-16">
                <div class="spaye-y-4">
                    ...
                    <div class="pb-2">
                        ...
                        <input type="text" id="text" name="" value="{{$datas->nama_usaha}}" class="bg-white mt-1 block w-full px-3 py-1 border border-gray-600 rounded shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="">
                    </div>
                    <div class="pb-2">
                        <label for="text" class="block text-sm font-normal text-gray-700">Alamat Usaha
                            <input type="text" id="text" name="" value="{{$datas->alamat_usaha}}" class="bg-white mt-1 block w-full px-3 py-1 border border-gray-600 rounded shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="">
                    </div>
                    <div class="pb-2">
                        <label for="nama" class="block text-sm font-normal text-gray-700">Bidang Usaha</label>
                        <input type="text" id="nama" name=""  value="{{$datas->bidang_usaha}}" class="bg-white mt-1 block w-full px-3 py-1 border border-gray-600 rounded shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="">
                    </div>
                </div>
                {{-- Upload Berkas --}}
                <div class="">
                    ...
                    @if ($datas->status != 'Ditolak')
                        <div class="flex justify-between mt-6 mx-auto w-[100%] pb-4">
                            <x-pop-up-tolak />
                            <form action="{{ route('verif.sku', $datas->id) }}" method="POST">
                                @csrf
                                <button type="submit"
...
<script>
    document.getElementById('submit-reject').addEventListener('click', function() {
        ...
        form.method = 'POST';
        form.action ='{{ route('reject.sku', $datas->id) }}'; // Pastikan route ini sudah ada di routes/web.php
