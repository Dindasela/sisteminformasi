            ...
            <div class="bg-[#F5F3F3] pb-16 p-6 rounded-lg shadow-lg">
                ...
                <form action="{{ route('pengumuman.update', $pengumuman->id) }}" method="POST"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="flex flex-col items-center justify-center border-gray-300 rounded-lg h-64 bg-[#D9D9D9]"
                            ...
                            <div class="text-center" id="fileDisplayArea">
                                @if ($pengumuman->image)
                                    <img src="{{ asset('storage/' . $pengumuman->image)}}" alt="Current Image"
                        ...
                        <div class="space-y-4 pb-4">
                            ...
                            <div>
                                <input type="date" id="tanggal" name="date"
                                    ...
                                    class="bg-[#F5F3F3] mt-1 block w-full px-3 py-2 border border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text">
                            </div>
                            <div>
                                <input type="text" id="tempat" name="place"
                                    value="{{ old('place', $pengumuman->place) }}"
                                    class="bg-[#F5F3F3] mt-1 block w-full px-3 py-2 border border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                    placeholder="Tempat">
                            </div>
                            <div>
                                <textarea id="isi" name="description" rows="4"
                                    class="bg-[#F5F3F3] mt-1 block w-full px-3 py-2 border border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                    placeholder="Isi Pengumuman">{{ old('description', $pengumuman->description) }}</textarea>
                            </div>
                            <div class="flex items-center space-x-4 mt-4">
                                <label class="py-1 flex items-center border border-gray-600 rounded-md pl-2 w-[50%]">
                                    <input type="radio" name="status" value="Terlaksana"
                                        {{ old('status', $pengumuman->status) == 'Terlaksana' ? 'checked' : '' }}
                                ...
                                <label class="py-1 flex items-center border border-gray-600 rounded-md pl-2 w-[50%]">
                                    <input type="radio" name="status" value="Belum Terlaksana"
                                        {{ old('status', $pengumuman->status) == 'Belum Terlaksana' ? 'checked' : '' }}
                                        class="form-radio text-indigo-600 bg-[#F5F3F3]">
                    ...
                    <div class="float-right text-center">
                        <button type="submit"
                            class="bg-[#2B2A4C] text-white px-4 py-1 rounded-md hover:bg-[#414066]">Update</button>
                    </div>
                </form>
            </div>
        </div>
